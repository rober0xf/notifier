...
func (s *Store) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	password, err := hash_password(user.Password)
	if err != nil {
	...
	if err := s.DB.Create(&user).Error; err != nil {
		error_message := strings.ToLower(err.Error())
		switch {
		case strings.Contains(error_message, "unique") || strings.Contains(error_message, "duplicate"):
			http.Error(w, "user already exists", http.StatusConflict)
			return
		case strings.Contains(error_message, "null") || strings.Contains(error_message, "required"):
...
func (s *Store) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	// update the password
	password, err := hash_password(inputUser.Password)
	if err != nil {
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
	}
